...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen p-4 md:p-6" style={{ background: C.bg }}>
      ...
      {tab === "linkedin" && li && (<>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <KPICard icon={Users} label="Total Network" value={li.totalConnections?.toLocaleString()} sub={`+${li.newLast7Days} this week Â· +${li.newLast30Days} this month`} color={C.primary} trend={`${li.newLast30Days} new in 30d`} up />
          ...
          <KPICard icon={MessageSquare} label="Messages" value={li.totalMessages?.toLocaleString()} sub={`${li.messagesSent?.toLocaleString()} sent Â· ${li.uniqueConversations?.toLocaleString()} conversations`} color={C.secondary} />
        </div>
        ...
        <div className="mt-4 rounded-lg px-4 py-3 text-xs" style={{ background: `${C.info}10`, color: C.muted }}>
          ...
          LinkedIn data last updated: {li.updatedAt} Â· Upload a new LinkedIn export to refresh
        </div>
      </>)}
